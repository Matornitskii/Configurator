...
export interface ModuleSpec{
  id:string; sku:string; modelId:string;
...
export interface BuildState{
  ...
  placed:{moduleId:string;x:number;y:number;rotation:0|90|180|270;}[];
...
export function computeTotals(placed:{x:number;y:number;moduleId:string}[], specs:Record<string,ModuleSpec>){
